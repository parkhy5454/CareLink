...
이름, 전화번호, 이메일, 가족 구성원 정보(이름, 관계, 출생연도), 예약 및 처방 내역 등 건강 관련 민감정보를 수집합니다.
...
4. 민감정보 처리 동의
회사는 예약·처방 내역 등 건강에 관한 정보(가족 건강정보 포함)를 개인정보보호법상 민감정보로 처리하며, 이 동의는 서비스 이용을 위해 필수입니다.
    ...
    <>
      ...
      <div className="screen">
        <div className="onboard onboard-scroll">
          ...
          {mode === 'signup' ? (
            <SignupForm onLoggedIn={onLoggedIn} onSwitchToLogin={() => setMode('login')} />
          ) : (
            <LoginForm onLoggedIn={onLoggedIn} onSwitchToSignup={() => setMode('signup')} />
          )}
        </div>
      </div>
    </>
...
function SignupForm({ onLoggedIn, onSwitchToLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    if (phone.replace(/[^0-9]/g, '').length < 9) return setError('휴대폰 번호를 정확히 입력해주세요')
    if (!email.includes('@')) return setError('올바른 이메일을 입력해주세요')
    if (password.length < 6) return setError('비밀번호는 6자 이상이어야 해요')
    if (!agree) return setError('이용약관 및 개인정보처리방침에 동의해주세요')
  ...
  return (
    <form className="onboard-actions signup-form" onSubmit={handleSubmit}>
      ...
      <label className="field-label">추천인 코드 (선택)</label>
      <input
        className="input-field"
        type="text"
        placeholder="친구에게 받은 코드가 있으면 입력하세요"
        value={referralCode}
      ...
      <label className="consent-row consent-inline">
        <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} />
        <span>
          <button type="button" className="consent-link" onClick={(e) => { e.preventDefault(); setExpanded(expanded === 'terms' ? null : 'terms') }}>
            이용약관
          </button>
          {' '}및{' '}
          <button type="button" className="consent-link" onClick={(e) => { e.preventDefault(); setExpanded(expanded === 'privacy' ? null : 'privacy') }}>
            개인정보처리방침(가족 건강정보 수집·이용 포함)
          </button>
          에 동의합니다.
        </span>
      </label>
      ...
      <button type="button" className="btn-more" onClick={onSwitchToLogin}>
        이미 계정이 있으신가요? 로그인
      </button>
    </form>
  )
...
function LoginForm({ onLoggedIn, onSwitchToSignup }) {
  ...
  return (
    <form className="onboard-actions" onSubmit={handleSubmit}>
      ...
      <button type="button" className="btn-more" onClick={onSwitchToSignup}>
        계정이 없으신가요? 회원가입
      </button>
